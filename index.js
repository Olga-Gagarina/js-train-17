...
function replaceText(word, replacement, text) {
  ...
  const result = text.replaceAll(word,replacement);
  return result;
...
function replaceWordOccurrences(str, word, newWord) {
  const regex = new RegExp(word, "g");
  const result = str.replaceAll(word, newWord);
  return result;
...
function checkFlags(regex) {
  const matches = [];
  const flags = regex.flags;
  if( flags.includes("i") == true){
    matches.push("ignoreCase ");
  }
  const sourced = regex.source;
  matches.push(sourced);
  return matches;
